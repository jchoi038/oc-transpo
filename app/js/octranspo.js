...
var targObj = "zoomPanObj",
	...
	contRat = typeof contRat !== "undefined" ? contRat : { x: 1, y: 1 };
...
function pan(cDims, pMPos, pEvt, e) {
	var pos = getPos(),
		...
		pPos = { x: ePos.x - pMPos.x + pos.x, y: ePos.y - pMPos.y + pos.y },
		mDims = { x: $("#" + moveName).width(), y: $("#" + moveName).height() };
	...
	if ($("#" + targObj).length > 0) {
		if ($(window).width() < 992 || $("html").is(".touchevents")) {
			...
			if (urlExists(originalSrc.replace("PNG/", "PNG/Thumb/"))) {
				$("#" + targObj).attr(
					"src",
					originalSrc.replace("PNG/", "PNG/Thumb/")
				);
			}
			$("#" + targObj).css("float", "none");
			$("#" + moveName).css("position", "static");
			$("#" + targObj).wrap(
				'<a href="' + originalSrc + '" target="_blank"></div>'
			);
			$("#zoomBtns").hide();
			return;
		}
		$(window).on("load", function () {
			var bDims = {
				...
				mobile =
					$("#" + mobileName).css("display") == "block"
						? true
						: false,
				bRat = bDims.y / bDims.x;
			(cRat = mobile ? { x: 1, y: bDims.y / bDims.x } : contRat),
				(cDims = { x: bDims.x / cRat.x, y: bDims.y / cRat.y }),
				(sFact = 1.3),
				(sFactS = 1.03),
				(dots = 10),
				(zLvl = 0),
...
function ocUtilities($) {
	...
	$('a[rel="noopener"]').each(function () {
		if (
			$(this).attr("href").indexOf("dev.octranspo.com") != -1 ||
			$(this).attr("href").indexOf("www.octranspo.com") != -1
		) {
			$(this).removeAttr("rel");
	...
	if (pageHash == "#news-240782") {
		window.location.replace(
			"/en/news/article/new-look-for-octranspo-website/"
		);
	}
	if (pageHash == "#news-240783") {
		window.location.replace(
			"/fr/nouvelles/article/refonte-du-site-octranspo/"
		);
	}
	$(".fr .news-pagination a").each(function () {
		$(this).attr(
			"href",
			$(this).attr("href").replace("/news", "/nouvelles")
		);
	...
	$(".en .link-new-window").each(function () {
		...
		$(this).append(
			'<span class="new-window-tooltip">Opens external website in a new window/tab.</span>'
		);
	});
	$(".fr .link-new-window").each(function () {
		$(this).find(".new-window-tooltip").remove();
		$(this).append(
			'<span class="new-window-tooltip">Ouvre le site Web externe dans une nouvelle fenÃªtre ou un nouvel onglet</span>'
		);
	});
...
function mobileMainNav($) {
	if ($(window).width() < 992) {
		...
		$(
			".main-nav li ul, .stage2-main-nav > li > ul, .stage2-main-nav li ul"
		).each(function () {
			$(this).hide();
			if ($(this).parent().is(".list-flexbox-column")) {
				...
				if (
					...
					$(this)
						...
						.children("a")
						.append(
							'<span class="expand" tabindex="0" role="button" aria-expanded="false">Expand</span>'
						);
				}
			} else {
		...
		$(".main-nav .expand, .stage2-main-nav .expand").click(function (e) {
			...
			if (
				$(this).is(".expanded") ||
				$(this).parent().parent().is(".subnav-open")
			) {
...
function nextTrips($) {
	...
	var favouritesInstructions =
		"Select the star beside a route to favourite it.";
	...
	if ($("body").is(".fr")) {
		...
		favouritesInstructions =
			"Pour ajouter un circuit Ã  vos favoris, cliquez sur lâ€™Ã©toile.";
	...
	$("#next-departure-stop").autocomplete({
		classes: { "ui-autocomplete": "next-trips-autocomplete" },
		source: function (request, response) {
			...
			if (searchTerm.length > 0) {
				$.getJSON(
					baseURL +
						"next_trips/stops?keyword=" +
						encodeURIComponent(request.term),
					function (data) {
						response(
							$.map(data.Gtfs, function (stop_data) {
								return {
									value:
										stop_data.stop_code +
										" - " +
		...
		close: function (event, ui) {
			if (!disableAutocompleteSearch) {
				getNextTripsByStopID(
					$(event.target).val(),
					"#next-departure-stop-route-ids"
				);
			} else {
				disableAutocompleteSearch = false;
			}
	...
	if ($('.next-trips-get-stops input[name="stop_id"]').length > 0) {
		$('.next-trips-get-stops input[name="stop_id"]')[0].oninvalid =
			function () {
				if ($("body").is(".fr")) {
					if (
						$('.next-trips-get-stops input[name="stop_id"]').val()
							.length < 4
					) {
	...
	$(".next-trips-get-routes").delegate(
		'input[type="checkbox"]',
		"keyup",
		function (e) {
			if (e.keyCode == "13") {
	...
	$(".next-trips-get-stops").submit(function (e) {
		e.preventDefault();
		getNextTripsByStopID(
			$(this).find('input[name="stop_id"]').val(),
			"#next-departure-stop-route-ids"
		);
	});
	...
	$(".next-trips").delegate(".refresh", "click", function (e) {
		e.preventDefault();
		$(
			'.next-trips-get-routes .next-trips-route input[type="checkbox"]:checked'
		)
			.trigger("click")
			.trigger("click");
	});
	...
	$("#next-trip").on("click", ".favourite-route", function (e) {
		...
		if (favouriteRoutes.indexOf(routeData) == -1) {
			...
			$('.favourite-route[data-routestop="' + routeData + '"]').addClass(
				"selected"
			);
		} else {
			var favouriteIndex = favouriteRoutes.indexOf(routeData);
			$(
				'.favourite-route[data-routestop="' + routeData + '"]'
			).removeClass("selected");
			favouriteRoutes.splice(favouriteIndex, 1);
		}
		Cookies.set("favourites", favouriteRoutes.toString(), { expires: 365 });
		favouritesWidget();
	});
	...
	$("#next-trip").on("click", ".remove-favourite", function (e) {
		routeData =
			$(this).prev().data("route") + "-" + $(this).prev().data("stop");
		var favouriteIndex = favouriteRoutes.indexOf(routeData);
		$('.favourite-route[data-routestop="' + routeData + '"]').removeClass(
			"selected"
		);
		favouriteRoutes.splice(favouriteIndex, 1);
		Cookies.set("favourites", favouriteRoutes.toString(), { expires: 365 });
		...
		if (
			$("#next-trip .favourites .favourite-route-nav-item").length &&
			$("#next-trip .favourites .favourite-route-nav-item").length < 6
		) {
			favouritesWidget();
		}
	});
...
function favouritesWidget() {
	...
	favouritesWidgetContent +=
		'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="22.824" viewBox="0 0 24 22.824"><path id="Icon_feather-star" data-name="Icon feather-star" d="M15,3l3.708,7.512L27,11.724l-6,5.844,1.416,8.256L15,21.924l-7.416,3.9L9,17.568,3,11.724l8.292-1.212Z" transform="translate(-3 -3)" fill="#707070"/></svg>';
	...
	if (favouriteRoutes.length == 0) {
		...
		for (var i = 0; i < favouriteRoutes.length; i++) {
			...
			if (typeof favouriteRouteArray[2] !== "undefined") {
				routeString =
					'<div class="favourite-route-nav-item"><span class="favourite-route-get" data-route="' +
					favouriteRouteArray[0] +
					'" data-stop="' +
					favouriteRouteArray[1] +
					"-" +
					favouriteRouteArray[2] +
					'">';
				routeString +=
					favouriteRouteArray[0] +
					favouritesAtSeparator +
					favouriteRouteArray[1] +
					"-" +
					favouriteRouteArray[2];
			} else {
			...
			routeString +=
				'</span><span class="remove-favourite" title="' +
				removeFavourite +
				'">';
			routeString +=
				'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">\x3c!--! Font Awesome Pro 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --\x3e<path d="M135.2 17.7C140.6 6.8 151.7 0 163.8 0H284.2c12.1 0 23.2 6.8 28.6 17.7L320 32h96c17.7 0 32 14.3 32 32s-14.3 32-32 32H32C14.3 96 0 81.7 0 64S14.3 32 32 32h96l7.2-14.3zM32 128H416V448c0 35.3-28.7 64-64 64H96c-35.3 0-64-28.7-64-64V128zm96 64c-8.8 0-16 7.2-16 16V432c0 8.8 7.2 16 16 16s16-7.2 16-16V208c0-8.8-7.2-16-16-16zm96 0c-8.8 0-16 7.2-16 16V432c0 8.8 7.2 16 16 16s16-7.2 16-16V208c0-8.8-7.2-16-16-16zm96 0c-8.8 0-16 7.2-16 16V432c0 8.8 7.2 16 16 16s16-7.2 16-16V208c0-8.8-7.2-16-16-16z"/></svg>';
	...
	if (favouriteRoutes.length > 5) {
		favouritesWidgetContent +=
			'<a class="show-more">' + moreFavouritestext + "</a>";
		favouritesWidgetContent +=
			'<a class="show-less">' + fewerFavouritestext + "</a>";
	}
...
function getNextTripsByStopID(stopID, targetElement, selectedRoute) {
	...
	var routeJSON = $.getJSON(
		baseURL + "next_trips/routes?stop_id=" + stopID,
		function (data) {
			...
			if (
				typeof data.GetRouteSummaryForStopResult.Error == "undefined" &&
				typeof data.GetRouteSummaryForStopResult.StopNo == "undefined"
			) {
				data.GetRouteSummaryForStopResult = {};
				data.GetRouteSummaryForStopResult.Error = "10";
			}
			if (
				typeof data.GetRouteSummaryForStopResult.Error !==
					"undefined" &&
				data.GetRouteSummaryForStopResult.Error
			) {
				if ($("body").is(".fr")) {
					switch (data.GetRouteSummaryForStopResult.Error) {
						case "1":
							$(".next-trips-get-routes").before(
								'<h4 class="error">ClÃ© API invalide</h4>'
							);
							break;
						case "2":
							$(".next-trips-get-routes").before(
								'<h4 class="error">Impossible d\'interroger la source de donnÃ©es</h4>'
							);
							break;
						case "10":
							$(".next-trips-get-routes").before(
								'<h4 class="error">NumÃ©ro d\'arrÃªt invalide.</h4>'
							);
							break;
						case "11":
							$(".next-trips-get-routes").before(
								'<h4 class="error">NumÃ©ro de route invalide.</h4>'
							);
							break;
						case "12":
							$(".next-trips-get-routes").before(
								'<h4 class="error">Stop ne dessert pas la route</h4>'
							);
							break;
						case "13":
							$(".next-trips-get-routes").before(
								'<h4 class="error">EntrÃ©e invalide</h4>'
							);
							break;
						case "14":
							$(".next-trips-get-routes").before(
								'<h4 class="error">Nom d\'arrÃªt invalide.</h4>'
							);
							break;
						default:
							$(".next-trips-get-routes").before(
					...
					switch (data.GetRouteSummaryForStopResult.Error) {
						case "1":
							$(".next-trips-get-routes").before(
								'<h4 class="error">Invalid API key</h4>'
							);
							break;
						case "2":
							$(".next-trips-get-routes").before(
								'<h4 class="error">Unable to query data source</h4>'
							);
							break;
						case "10":
							$(".next-trips-get-routes").before(
								'<h4 class="error">Invalid stop number</h4>'
							);
							break;
						case "11":
							$(".next-trips-get-routes").before(
								'<h4 class="error">Invalid route number</h4>'
							);
							break;
						case "12":
							$(".next-trips-get-routes").before(
								'<h4 class="error">Stop does not service route</h4>'
							);
							break;
						case "13":
							$(".next-trips-get-routes").before(
								'<h4 class="error">Invalid input</h4>'
							);
							break;
						case "14":
							$(".next-trips-get-routes").before(
								'<h4 class="error">Invalid stop name</h4>'
							);
							break;
						default:
							$(".next-trips-get-routes").before(
			...
			$(".next-trips span.loading").remove();
			$(".next-trips .ui-autocomplete-input").removeClass(
			...
			if (typeof data.GetRouteSummaryForStopResult !== "undefined") {
				...
				if (
					typeof data.GetRouteSummaryForStopResult.Routes ==
					"undefined"
				) {
					return false;
				}
				$(".next-trips-get-routes").show();
				$(".stop_id_outer").append(
					'<button class="next-trips-reset clear">clear</button>'
				);
				if (
					typeof data.GetRouteSummaryForStopResult.Routes.Route
						.RouteNo != "undefined"
				) {
					if (
						$.inArray(
							data.GetRouteSummaryForStopResult.Routes.Route
								.RouteNo,
							displayedRoutes
						) == -1
					) {
						...
						$(targetElement).append(
							'<div class="next-trips-route col-2"><input type="checkbox" name="route_id" value="' +
								data.GetRouteSummaryForStopResult.Routes.Route
									.RouteNo +
								'" id="next-trips-' +
								data.GetRouteSummaryForStopResult.Routes.Route
									.RouteNo +
								'"><label for="next-trips-' +
								data.GetRouteSummaryForStopResult.Routes.Route
									.RouteNo +
								'">' +
								data.GetRouteSummaryForStopResult.Routes.Route
					...
					for (
						...
						if (
							...
							$(targetElement).append(
								'<div class="next-trips-route col-2"><input type="checkbox" name="route_id" value="' +
									data.GetRouteSummaryForStopResult.Routes
										.Route[i].RouteNo +
									'" id="next-trips-' +
									data.GetRouteSummaryForStopResult.Routes
										.Route[i].RouteNo +
					...
					.change(function () {
						...
						$(targetElement)
							...
							.each(function () {
								...
								var nextTripsJSON = $.getJSON(
									baseURL +
										"next_trips/next_trips?stop_id=" +
										stopID +
										"&route_id=" +
										$(this).val(),
									function (nextTripsData) {
										var returnString = "",
											nextTrips = false;
										var routeStopString;
										...
										console.log(
											nextTripsData
												.GetNextTripsForStopResult.Route
												.RouteDirection
										);
										if (
											typeof nextTripsData
												.GetNextTripsForStopResult.Route
											...
											if (
												...
												if (
													...
													nextTripsData.GetNextTripsForStopResult.Route.RouteDirection.RouteNo =
														currentRouteIDFallback;
													currentRouteID =
														currentRouteIDFallback;
												...
												routeNoIcon =
													'<span class="favourite-route favourite-route-single ' +
													favouriteIsSelectedString +
													'" data-routestop="' +
													routeStopString +
													'" title="' +
												...
												if (nextTrips) {
													returnString =
														'<div class="route-result" data-modified="' +
														Date.now() +
														'" data-route="' +
														nextTripsData
															.GetNextTripsForStopResult
													...
													console.log(
														"no trips single direction " +
															currentRouteID
													);
													returnString =
														'<div class="route-result no-results" data-modified="' +
														Date.now() +
														'" data-route="' +
														nextTripsData
															.GetNextTripsForStopResult
												...
												for (
													...
													if (nextTrips) {
														returnString +=
															'<div class="route-result-inner"><strong>';
														returnString +=
															nextTripsData
																.GetNextTripsForStopResult
																.Route
											...
											console.log(
												"no results " + currentRouteID
											);
											returnString +=
												'<div class="route-result"><strong>';
											returnString +=
												currentRouteID +
												"</strong>: " +
												$(
										...
										$(
											".next-trip-results .route-result"
										).each(function () {
											if (
												$(this).data("route") ==
												currentRouteIDLocal
						...
						$(".next-trip-results .route-result").addClass(
							"remove"
						);
						$(".next-trip-results .route-result").each(function () {
							console.log(selectedRoutes);
							if (
								selectedRoutes.indexOf(
									parseFloat($(this).data("route"))
								) != -1
							) {
				...
				if (selectedRoute != undefined) {
					...
					if ($(targetCheckbox).length) {
						...
						$(".next-trip-results").append(
							'<div class="route-result no-results">' +
								noServiceLabel +
								"</div>"
						);
						$(".next-trip-results").show();
...
function getFormattedNextTrips(nextTrips, routeLabel) {
	...
	for (var i = 0; i < nextTrips.length; i++) {
		hasCoords = false;
		var tripArrivaltime = new Date(
			currentTime.getTime() + nextTrips[i].AdjustedScheduleTime * 6e4
		);
		if (nextTrips[i].AdjustmentAge > -1) {
			hasCoords = true;
		}
		formattedTrips +=
			tripArrivaltime.getHours() +
			":" +
			pad_number(tripArrivaltime.getMinutes(), 2);
		if (hasCoords) {
			formattedTrips += "*";
		}
		if (nextTrips[i].TripDestination != routeLabel) {
			formattedTrips +=
				'<span class="info" title="Destination ' +
				nextTrips[i].TripDestination +
...
function tabNavigation($) {
	...
	$('[role="tab"]').click(function (e) {
		...
		$(this)
			.parent()
			.find('[role="tab"]')
			.removeClass("aria-selected", "false");
		$(this).parent().find('[role="tab"]').attr("aria-selected", "false");
		$(this)
			.parent()
			.find('[role="tab"]')
			.removeClass("aria-current", "false");
	...
	if (pageHash.length > 0) {
		$("a.tablink").each(function () {
			if (
				$(this).attr("href") == pageHash ||
				$($(this).attr("href")).find(pageHash).length > 0
			) {
				openTabContent(null, $(this).attr("href").replace("#", ""));
				$(this).addClass("active");
			}
...
function feedbackButton($) {
	...
	$(".btn-feedback .close").click(function () {
		Cookies.set("feedback-btn-hide", true, { expires: 1 });
		$(this).parent().remove();
	});
}
function homepageAlerts($) {
	$(
		".homepage-widgets__widget #alerts-routes a, .homepage-widgets__widget #alerts-stations a"
	).click(function (e) {
		var lang = "en";
		if ($("body").is(".fr")) {
			lang = "fr";
		...
		$(this)
			...
			.parent()
			.find("a")
			.removeClass("tab-content__item__link--active");
		$(this).addClass("tab-content__item__link--active");
		e.preventDefault();
		if ($(this).data("route")) {
			$("#alerts-routes-content").load(
				baseURL +
					"alerts/filter?alertlang=" +
					lang +
					"&route=" +
					$(this).data("route"),
				function () {}
			...
			$("#alerts-routes-content").load(
				baseURL + "alerts/filter?alertlang=" + lang,
				function () {}
			);
		}
	});
	$(
		".homepage-widgets__widget #alerts-routes .more a, .homepage-widgets__widget #alerts-stations .more a"
	).unbind("click");
	$(".homepage-widgets__widget #alerts-routes .more a").click(function (e) {
		...
		if ($(this).parent().is(".less")) {
			$(window).scrollTop(
				$(".homepage-widgets__widget #alerts-routes").offset().top - 50
			);
			$(
				".homepage-widgets__widget #alerts-routes .alert-route-more-visible"
			).each(function () {
				$(this)
					.addClass("alert-route-more")
					.removeClass("alert-route-more-visible");
			});
		} else {
			$(
				".homepage-widgets__widget #alerts-routes .alert-route-more"
			).each(function () {
				$(this)
					.addClass("alert-route-more-visible")
					.removeClass("alert-route-more");
			});
		}
	});
	$(".homepage-widgets__widget #alerts-stations .more a").click(function (e) {
		...
		if ($(this).parent().is(".less")) {
			$(window).scrollTop(
				$(".homepage-widgets__widget #alerts-stations").offset().top -
					50
			);
			$(
				".homepage-widgets__widget #alerts-stations .alert-route-more-visible"
			).each(function () {
				$(this)
					.addClass("alert-route-more")
					.removeClass("alert-route-more-visible");
			...
			$(
				".homepage-widgets__widget #alerts-stations .alert-route-more"
			).each(function () {
				$(this)
					.addClass("alert-route-more-visible")
					.removeClass("alert-route-more");
	...
	$(
		".homepage-widgets__widget--alerts--alerts-inline #alerts-routes a, .homepage-widgets__widget--alerts--alerts-inline #alerts-stations a"
	).click(function (e) {
		$(this)
			.parent()
			.parent()
		...
		if ($(this).data("route")) {
			var selectedRoute = $(this).data("route"),
				routeDisplayCount = 0;
			$(
				".col--alerts-isn .alert, .col--alerts-cancellations .alert, .col--alerts-detours .alert, .col--alerts-stations .alert"
			).hide();
			$(".col--alerts-cancellations .alert").each(function () {
				...
				$(".col--alerts-cancellations .display-count").html(
					routeDisplayCount
				);
				if (routeDisplayCount < 1) {
					$(".col--alerts-cancellations .display-count")
						.parent()
						.hide();
				} else {
					$(".col--alerts-cancellations .display-count")
			...
			$(".col--alerts-detours .alert").each(function () {
				...
				$(".col--alerts-detours .display-count").html(
					routeDisplayCount
				);
				if (routeDisplayCount < 1) {
			...
			$(".col--alerts-stations .alert").each(function () {
				...
				$(".col--alerts-stations .display-count").html(
					routeDisplayCount
				);
				if (routeDisplayCount < 1) {
			...
			$(
				".col--alerts-stations .alert, .col--alerts-other .alert, .col--alerts-isn .alert"
			).hide();
			$(".col--alerts-cancellations .alert").each(function () {
			...
			$(".col--alerts-other .alert").each(function () {
				...
				$(".col--alerts-other .display-count").html(
					stationDisplayCount
				);
				if (stationDisplayCount < 1) {
			...
			$(
				".col--alerts-cancellations .alert, .col--alerts-detours .alert"
			).show();
		}
	});
	var pageHash = window.location.hash;
	$(".tabs--alerts > a").click(function () {
		if ($(this).is(".alert-tab-stations")) {
			...
			$(".col--alerts-stations .display-count").html(
				$(".col--alerts-stations .alert").length
			);
	...
	$(
		".homepage-widgets__widget--alerts--alerts-inline .alert-tab-routes"
	).trigger("click");
	$(".homepage-widgets__widget--alerts--alerts-inline").delegate(
		".alert a",
		"keyup",
		function (e) {
			if (e.keyCode == "13") {
				...
				if (typeof targetAnchor[1] !== "undefined") {
					$('div[data-alert="' + targetAnchor[1] + '"]').each(
						function () {
							$(".alert h3")
								.removeClass("from-anchor")
								.removeClass("expanded");
							$(".alert .alert-content").hide();
							$(this)
								.find("h3")
								.addClass("from-anchor")
								.addClass("expanded");
	...
	$(
		".homepage-widgets__widget--alerts--alerts-inline .btn--all-alerts"
	).unbind("click");
	$(
		".homepage-widgets__widget--alerts--alerts-inline .btn--all-alerts"
	).click(function (e) {
		...
		$(this).addClass("tab-content__item__link--active");
		$(
			".col--alerts-cancellations, .col--alerts-detours, .col--alerts-stations, .col--alerts-other"
		).each(function () {
			var alertCount = 0;
			$(this).find("h2").show();
...
function alertsToggle($) {
	...
	if (pageHash.length > 1) {
		$(".alert").each(function () {
			if (
				"#" + $(this).data("alert") == pageHash ||
				$(this).data("alert-id") == pageHash.replace("#alerts-", "") ||
				$(this).data("alert-id") == pageHash.replace("#alert-", "")
			) {
				if (
					$(this).closest(".col--alerts").is(".col--alerts-stations")
				) {
	...
	$("header .isn a, .alert-content a").click(function (e) {
		...
		if (isnHref.length > 1) {
			targetAlertID = isnHref[1]
				.replace("alert-", "")
				.replace("alerts-", "");
			var alertSelected = false;
			$('.alert[data-alert-id="' + targetAlertID + '"]').each(
				function () {
					if (!alertSelected) {
						e.preventDefault();
						$(this)
...
function formValidation($) {
	$("body").delegate(
		...
		function (e) {
			$(this)[0].setCustomValidity("");
			if (
				($(this)[0].validity.valueMissing ||
					$(this)[0].validity.tooShort) &&
				$("body").is(".fr")
			) {
			...
			if ($(this)[0].validity.patternMismatch && $("body").is(".fr")) {
				$(this)[0].setCustomValidity(
					"CaractÃ¨res clÃ©s non autorisÃ©s."
				);
			}
			if (
				$(this)[0].validity.typeMismatch &&
				$(this).attr("type") == "url" &&
				$("body").is(".fr")
			) {
				$(this)[0].setCustomValidity(
					"S'il vous plaÃ®t entrer une URL."
				);
			}
		}
	);
...
function accordionToggle($) {
	...
	var i;
	$(".accordion-container .accordion, .faq-accordion .accordion").each(
		function () {
			$(this).click(function (e) {
				console.log("click");
...
function destinationsCategories($) {
	...
	if (
		$(".destinations-category-form").length > 0 &&
		$(".destinations-category-form").data("selected-categories")
	) {
		var destinationsSelectedCategories = $(
			".destinations-category-form"
		).data("selected-categories");
		if (typeof destinationsSelectedCategories == "number") {
			destinationsSelectedCategories =
				destinationsSelectedCategories.toString();
		}
		destinationsSelectedCategories =
			destinationsSelectedCategories.split("|");
		$('.destinations-category-form input[type="checkbox"]').each(
			function () {
				if (
					destinationsSelectedCategories.indexOf($(this).val()) != -1
				) {
	...
	if ($("body").is(".fr")) {
		$.getJSON(
			baseURL + "/fr/ottawa-destinations/venues-list",
			function (data) {
				venuesList = data.venues;
				$("#search-venue").autocomplete({
					classes: { "ui-autocomplete": "search-venue-autocomplete" },
		...
		$.getJSON(
			baseURL + "/en/ottawa-destinations/venues-list",
			function (data) {
				venuesList = data.venues;
				$("#search-venue").autocomplete({
					classes: { "ui-autocomplete": "search-venue-autocomplete" },
...
function submitApplicationForm($) {
	$(
		'input[name="app_download_link_android"], input[name="app_download_link_apple"], input[name="app_download_link_other"]'
	)
		.parent()
		.hide();
	$(
		'input[name="app_download_link_android"], input[name="app_download_link_apple"], input[name="app_download_link_other"], input[name="developer_url"]'
	).attr("type", "url");
	...
	$(".fr .channel_form label").each(function () {
		$(this).html(
			$(this)
				.html()
				.replace("Description", "Description de lâ€™application")
		);
		$(this).html($(this).html().replace("App Platform", "Plateforme"));
		$(this).html(
			$(this)
				.html()
				.replace("App Icon", "Capture dâ€™Ã©cran de lâ€™application")
		);
		$(this).html(
			$(this).html().replace("App Image", "IcÃ´ne de lâ€™application")
		);
		$(this).html(
			$(this)
				.html()
				.replace("Free or Paid app?", "Gratuite ou Ã  vendre?")
		);
		$(this).html(
			$(this).html().replace("Developer Name", "Nom du dÃ©veloppeur")
		);
		$(this).html(
			$(this)
				.html()
				.replace(
					...
					"Adresse courriel du dÃ©veloppeur"
				)
		);
		$(this).html(
			$(this)
				...
				.replace(
					"Developer URL",
					"Site Web du dÃ©veloppeur (adresse URL)"
				)
		);
	});
	...
	if ($(".app").length > 0) {
		$(".app").each(function () {
			if (pageHash == "#" + $(this).data("entry-id")) {
				console.log(
					'.tablink a[href="#' + $(this).parent().attr("id") + '"]'
				);
				$(
					'a.tablink[href="#' + $(this).parent().attr("id") + '"]'
				).trigger("click");
				$(window).scrollTop($(this).offset().top);
				return false;
			}
...
function lrtLaunchNavigation($) {
	$(".en.welcome-aboard-menu .main-nav .red-nav-item > a").html(
		'Welcome Ab<span class="lrt-nav-o">o</span>ard <span class="expand">Expand</span>'
	);
	$(".fr.welcome-aboard-menu .main-nav .red-nav-item > a").html(
		'Bienvenue Ã  b<span class="lrt-nav-o">o</span>rd <span class="expand">Expand</span>'
	);
	$(".en.welcome-aboard-menu .main-nav .o-line-nav-item > a").html(
		'The <span class="lrt-nav-o">o</span>-Line'
	);
	$(".fr.welcome-aboard-menu .main-nav .o-line-nav-item > a").html(
		'L\'<span class="lrt-nav-o">o</span>-Ligne'
	);
	$(".welcome-aboard-menu .main-nav .red-nav-item > a .expand").click(
		function (e) {
			$(this)
				.parent()
				.parent()
...
function lrtLaunchCountdown($) {
	if ($("body").is(".homepage-lrt-countdown")) {
		if ($(".lrt-launch-countdown-complete").length > 0) {
			$("#lrt-launch-countdown").countdown(endDate, function (event) {
				if ($("body").is(".fr")) {
					$(this).html(
						event.strftime(
							'<div class="date-segment"><div class="number">%D</div><div class="label">jours</div></div><div class="date-segment"><div class="number">%H</div><div class="label">heures</div></div><div class="date-segment"><div class="number">%M</div><div class="label">minutes</div></div><div class="date-segment"><div class="number">%S</div><div class="label">secondes</div></div>'
						)
					);
				} else {
					$(this).html(
						event.strftime(
							'<div class="date-segment"><div class="number">%D</div><div class="label">days</div></div><div class="date-segment"><div class="number">%H</div><div class="label">hours</div></div><div class="date-segment"><div class="number">%M</div><div class="label">minutes</div></div><div class="date-segment"><div class="number">%S</div><div class="label">seconds</div></div>'
						)
					);
				}
				if (event.elapsed == true && countdownComplete == false) {
					...
					if ($("body").is(".fr")) {
						countdownHTML =
							'<div class="date-segment date-segment-full"><div class="complete">Bienvenue Ã  bord!</div></div>';
					} else {
						countdownHTML =
							'<div class="date-segment date-segment-full"><div class="complete">Welcome aboard!</div></div>';
					}
					$("#lrt-launch-countdown").attr(
						"id",
						"lrt-launch-countdown-old"
					);
					$("#lrt-launch-countdown-old").after(
						'<div class="countdown lrt-launch-countdown-complete" id="lrt-launch-countdown">' +
							countdownHTML +
							"</div>"
					);
					$("#lrt-launch-countdown-old").remove();
					$("#lrt-launch-countdown-mobile").attr(
						"id",
						"lrt-launch-countdown-mobile-old"
					);
					$("#lrt-launch-countdown-mobile-old").after(
						'<div class="countdown" id="lrt-launch-countdown-mobile">' +
							countdownHTML +
							"</div>"
					);
					$("#lrt-launch-countdown-mobile-old").remove();
					$(
						"#lrt-launch-countdown-old, #lrt-launch-countdown-mobile-old"
					).hide();
					console.log("test:");
					console.log(
						$(
							".en #lrt-launch-countdown, .en #lrt-launch-countdown-mobile"
						).html()
					);
			...
			$("#lrt-launch-countdown").countdown(
				"2019/09/14 14:00:00",
				function (event) {
					if ($("body").is(".fr")) {
						$(this).html(
		...
		if ($(".lrt-launch-countdown-complete").length > 0) {
			$("#lrt-launch-countdown-mobile").countdown(
				endDate,
				function (event) {
					$(this).html(
						event.strftime(
							'<div class="date-segment">%D:%H:%M:%S</div>'
						)
					);
				}
			);
...
function lrtCountdownComplete($) {
	...
	$("#homepage-widget-confetti").animate(
		...
		{
			duration: 2e3,
			specialEasing: { maxHeight: "easeout" },
			complete: function () {},
		}
	);
	setTimeout(function () {
	...
	$(".homepage-widget-wrap, #homepage-widget-confetti").on(
		"click scroll keyup",
		function (e) {
...
function serviceChangeNotification($) {
	$(".modal-service-change .close").click(function () {
		var serviceChangeCookieExpiration = new Date(
	...
	if ($(window).width() < 992) {
		$(".modal-service-change a").css(
			"height",
			$(".modal-service-change").height() + "px"
		);
	}
}
function homepageParallax($) {
	if ($("#parallax-container").length > 0) {
		...
		var scrollRatio =
			($(window).scrollTop() - parallaxContainer.offset().top) /
			parallaxContainer.height();
		var trainOffset =
			containerWidth * 0.5 - containerWidth * 0.5 * (scrollRatio * 0.9);
		var trainTwoOffset =
			containerWidth * 0.5 -
			(containerWidth * 0.5 + trainTwoWidth) * (scrollRatio * 0.65);
		var busOffset =
			containerWidth * 0.7 -
			(containerWidth * 0.7 + busWidth) * (scrollRatio * 0.2);
		$("#train-scroll").css(
			"margin-right",
			"-" + containerWidth * 0.3 + "px"
		);
		$("#train-two-scroll").css(
			"margin-left",
			"-" + containerWidth * 0.1 + "px"
		);
		$("#bus-scroll").css("margin-left", "-" + containerWidth * 0.05 + "px");
		...
		$(window).scroll(function () {
			...
			var parallaxContainerTop = parallaxContainer.offset().top,
				parallaxContainerBottom =
					parallaxContainer.offset().top + parallaxContainer.height(),
				windowScroll = $(window).scrollTop();
			var scrollPercentage =
				(($(window).scrollTop() - parallaxContainerTop) /
					parallaxContainer.height()) *
				100;
			var scrollRatio =
				($(window).scrollTop() - parallaxContainerTop) /
			...
			if (windowScroll > 0 && windowScroll < parallaxContainerBottom) {
				trainOffset =
					containerWidth * 0.5 -
					containerWidth * 0.5 * (scrollRatio * 0.9);
				trainTwoOffset =
					containerWidth * 0.5 -
					(containerWidth * 0.5 + trainTwoWidth) *
						(scrollRatio * 0.65);
				busOffset =
					containerWidth * 0.7 -
					(containerWidth * 0.7 + busWidth) * (scrollRatio * 0.2);
...
function logo_train() {
	if ($("body").is(".transit-150")) {
		setTimeout(function () {
			...
			$("#logo150").animate(
				{ left: "+=133", easing: "easeOutCirc" },
				1900
			);
			setTimeout(function () {
				$("#logo150").animate({ opacity: "0" }, 1e3, function () {
					$("#logo").animate({ opacity: "1" }, 1e3, function () {
						logo_train();
					});
				});
			}, 3e3);
...
function stage2Nav() {
	var pageHash = window.location.hash;
	if ($(window).width() > 991) {
		$(".stage2-main-nav > li > ul, .stage2-main-nav > li > .subnav").each(
			function () {
				$(this).parent().addClass("has-subnav");
				$(this)
					.parent()
	...
	$(".stage2-main-nav > li").each(function () {
		if ($(this).find("ul").length) {
			...
			$(this)
				.find(".subnav")
				.append(
					'<div class="list-flexbox"><div class="list-flexbox-column list-flexbox-column-1"><ul></ul></div><div class="list-flexbox-column list-flexbox-column-2"><ul></ul></div><div class="list-flexbox-column list-flexbox-column-3"><ul></ul></div></div>'
				);
			$(this)
				.find(".subnav")
				.append(
					'<div class="list-flexbox list-flexbox-bottom"><div class="list-flexbox-column list-flexbox-bottom-column-1"><ul></ul></div><div class="list-flexbox-column list-flexbox-bottom-column-2"><ul></ul></div><div class="list-flexbox-column list-flexbox-bottom-column-3"><ul></ul></div></div>'
				);
		}
	});
	...
	$(".stage2-main-nav > li > ul > li").each(function () {
		if ($(this).is(".col3")) {
			$(this).appendTo(
				$(this).parent().parent().find(".list-flexbox-column-3 > ul")
			);
		} else if ($(this).is(".col2")) {
			$(this).appendTo(
				$(this).parent().parent().find(".list-flexbox-column-2 > ul")
			);
			...
			$(this).appendTo(
				$(this)
					.parent()
					.parent()
					.find(".list-flexbox-bottom-column-3 > ul")
			);
			...
			$(this).appendTo(
				$(this)
					.parent()
					.parent()
					.find(".list-flexbox-bottom-column-2 > ul")
			);
			...
			$(this).appendTo(
				$(this)
					.parent()
					.parent()
					.find(".list-flexbox-bottom-column-1 > ul")
			);
		} else {
			$(this).appendTo(
				$(this).parent().parent().find(".list-flexbox-column-1 > ul")
			);
		}
	});
	...
	$(".stage-2 a.tab").click(function () {
		...
		if ($(targetElement).find(".photo-carousel-container").length) {
			$(targetElement)
				.parent()
				.find(".navigation-buttons-slider.prev")
				.trigger("click");
		}
	});
		...
		if (e.keyCode == 9 && $(this).is(".no-scroll")) {
			setTimeout(function () {
				...
				if ($(":focus").closest(".stage2-nav-section").length <= 0) {
					$(".mobile-nav-min-subsection .navbar-toggler").trigger(
						"click"
					);
				}
			}, 0);
	...
	$(".stage-2 .stage2-main .container-prev-next .stage2-prev-next a").unbind(
		"click"
	);
	$(
		".stage-2 .stage2-main .container-prev-next .stage2-prev-next a, .stage2-main-nav .subnav-item a"
	).click(function (e) {
		var pageHash = window.location.hash;
		...
		if (linkHref.indexOf(currentURL) != -1) {
			...
			if ($(linkAnchor).length) {
				if ($('[role="tablist"] a[href="' + linkAnchor + '"]').length) {
					$('[role="tablist"] a[href="' + linkAnchor + '"]').trigger(
						"click"
					);
				}
				scrollToValue = $(linkAnchor).offset().top;
				scrollToValue -= $(linkAnchor)
					.parent()
					.find('[role="tablist"]')
					.height();
	...
	if ($("body.stage-2").length && pageHash) {
		if ($('a[href="' + pageHash + '"]').length) {
			$(window).scrollTop(
				$('a[href="' + pageHash + '"]').offset().top - 150
			);
		}
		if (
			$(".mobile-tabcontent .accordion").length ||
			$(".mobile-tabcontent .tab-anchor-inner").length
		) {
			var parentTabID;
			if ($(pageHash).is(".accordion")) {
				parentTabID = $(pageHash)
					.closest(".mobile-tabcontent")
					.attr("id");
				if (parentTabID) {
					parentTabID = "#" + parentTabID;
					$('[role="tablist"] a[href="' + parentTabID + '"]').trigger(
						"click"
					);
...
function stage2NavExpandButtons() {
	...
	if (
		$(".stage2-main-nav").length &&
		$(".stage2-main-nav .expand").length == 0
	) {
		$(".stage2-main-nav a").each(function () {
			if ($(this).parent().parent().find("ul").length) {
				...
				if (controlsID) {
					$(this).after(
						'<button class="expand" tabindex="0" aria-controls="' +
							controlsID +
							'">' +
							expandBtnText +
							"</span></button>"
					...
					$(this).after(
						'<button class="expand" tabindex="0"><span class="visuallyhidden">' +
							expandBtnText +
							"</span></button>"
					);
				}
	...
	$(".stage2-main-nav .expand").click(function () {
		...
		$(".stage2-main-nav .main-nav-link-continer + ul")
			.not(".active")
			.hide();
...
function stage2LineNumbers() {
	if ($(".lines span").length) {
		$(".lines span").each(function () {
			$(this).html(
				'<span class="visuallyhidden">' + $(this).html() + "</span>"
			);
		});
	}
}
function stage2StationImages() {
	if ($(".destination-large").length) {
		$(".destination-large .inner").each(function () {
			if ($(this).find(".overlay").length == 0) {
				$(this).html(
					'<div class="overlay">' + $(this).html() + "</div>"
				);
			}
		});
	}
	$(".row-destinations .destination-list a").click(function (event) {
		...
		$(this)
			.parent()
			.parent()
			.find(".destination-large .inner h2")
			.html($(this).html());
		var imageURL = "url(" + $(this).attr("data-image") + ")";
		$(this)
			.parent()
			.parent()
			.find(".destination-large .inner")
	...
	if ($(".hero-slider").length) {
		$(".hero-slider").each(function () {
			var sliderElement = $(this),
				...
				slideHeight = 0;
			sliderElement
				.find(".slider-controls .total-slides")
				.html(sliderElement.find(".slide").length);
			sliderElement
				.find(".slider-controls .slide-control.prev")
				.click(function () {
					previousSlide(sliderElement);
				});
			sliderElement
				.find(".slider-controls .slide-control.next")
				.click(function () {
			...
			sliderElement.find(".slide").each(function () {
				...
				if ($(this).is(".active")) {
					sliderElement
						.find(".slider-controls .slider-pagination")
						.append(
							'<a href="#' +
								$(this).attr("id") +
								'" class="current">' +
			...
			sliderElement
				.find(".slider-controls .slider-pagination a")
			...
			sliderElement
				.find(".slider-controls .pause-button")
				.click(function (event) {
					event.preventDefault();
					$(this).toggleClass("paused");
					if (heroSliderTimerPaused) {
						heroSliderTimer = setInterval(function () {
							$(".hero-slider .slide-control.next").trigger(
								"click"
							);
						}, 3e3);
						heroSliderTimerPaused = false;
			...
			sliderElement
				.find(
					".slider-controls .slide-control, .slider-controls .slider-pagination a, .slider-controls .pause-button"
				)
...
function previousSlide(element) {
	...
	element
		.find(".slider-controls .slide-number .current-slide")
		.html(newSlide);
	element
		.find(".slider-controls .slider-pagination a")
		.removeClass("current");
	element
		.find(
			".slider-controls .slider-pagination a:nth-child(" + newSlide + ")"
		)
		.addClass("current");
}
...
function stage2MobileSlider() {
	...
	if (
		$(window).width() < 992 &&
		element.length &&
		$(".info-bubbles .slider-canvas").length == 0
	) {
		...
		$(".info-bubbles").each(function () {
			...
			element.children().each(function () {
				...
				paginationContent +=
					'<span class="paginate">' + slideCount + "</span>";
			...
			element.html(
				'<div class="slider-canvas" style="width: ' +
					totalWidth +
					'px;" data-current-slide="1">' +
					element.html() +
					"</div>"
			);
			element.append(
				'<span class="mobile-slider-nav previous">Previous</span>'
			);
			...
			element
				.find(".mobile-slider-pagination .paginate:first-child")
				.addClass("current");
...
function mobileSetSlide(element, position) {
	...
	if (targetSlide.length) {
		...
		element
			.find(".slider-canvas")
			.data("current-slide", position)
			.css("margin-left", targetLeft);
	}
}
...
function stage2SearchAnimation() {
	$("#searchBtnBar").click(function () {
		if ($('#desktop-search-form input[type="text"]').width() < 1) {
			$('#desktop-search-form input[type="text"]').css(
				"visibility",
				"visible"
			);
			$('#desktop-search-form input[type="text"]').css(
...
function stage2TimelineSlickDates() {
	if ($(".stage2-timeline-carousel")) {
		var transcribeValue1 = $("#stage2-timeline-transcribe-data1").data(
			"transcribe-value"
		);
		var transcribeValue2 = $("#stage2-timeline-transcribe-data2").data(
			"transcribe-value"
		);
		var transcribeValue3 = $("#stage2-timeline-transcribe-data3").data(
			"transcribe-value"
		);
		var slickDates = document.querySelectorAll(
			".stage2-timeline-carousel .slick-dots li button"
		);
...
function stage2HomeTimeline() {
	if ($(".stage2-timeline-carousel")) {
		if ($(".stage2-timeline-carousel ul.slick-dots")) {
			$(
				"<article class='top-line-circle-container dates-line-map'><section></section><div class='carousel-top-line carousel-top-line--one'></div><section></section><div class='carousel-top-line carousel-top-line--two'></div><section></section></article>"
			).appendTo(
				".stage2-timeline-carousel .timeline-carousel__item-wrapper"
			);
			$(".stage2-timeline-carousel ul.slick-dots").addClass(
				"dates-line-map"
			);
			$(".stage2-timeline-carousel ul.slick-dots").attr("role", "group");
			$(".stage2-timeline-carousel ul.slick-dots").attr(
				"aria-label",
				"Choose slide to display."
			);
			$("<div></div>")
				.addClass("line-circle line-circle--one")
				.appendTo(".stage2-timeline-carousel .timeline-carousel__item");
			$("<div></div>")
				.addClass("line-circle line-circle--two")
				.appendTo(".stage2-timeline-carousel .timeline-carousel__item");
			$("<div></div>")
				.addClass("line-circle line-circle--three")
				.appendTo(".stage2-timeline-carousel .timeline-carousel__item");
			$(".stage2-timeline-carousel .dates-line-container").append(
				$(
					".stage2-timeline-carousel .slick-dots, .stage2-timeline-carousel .top-line-circle-container"
				)
			);
			$("<section class='top-line-circle'></section>").appendTo(
				".stage2-timeline-carousel .slick-dots li"
			);
			$(".top-line-circle").each(function (i) {
				$(this).addClass("top-line-circle--" + i++);
			});
		}
		$(".dates-line-map").wrapAll(
			"<section class='dates-line-map-container' />"
		);
	}
}
function stage2HomeTimelineHoverEffects() {
	...
	if (hasTouchScreen === false || $(window).width() > 992) {
		...
		if ($(".stage2-timeline-carousel")) {
			if (
				$(slickDates[0]).hasClass("timeline-active") ||
				$(slickDates[1]).hasClass("timeline-active") ||
				$(slickDates[2]).hasClass("timeline-active")
			) {
				return 0;
			} else {
				$(".stage2-timeline-carousel .top-line-circle--0").mouseenter(
					function () {
						$(
							".stage2-timeline-carousel .top-line-circle--0"
						).addClass("timeline-circle-hover");
						$(slickDates[0]).addClass("timeline-date-hover");
					}
				);
				$(".stage2-timeline-carousel .top-line-circle--0").mouseleave(
					function () {
						$(slickDates[0]).removeClass("timeline-date-hover");
						$(
							".stage2-timeline-carousel .top-line-circle--0"
						).removeClass("timeline-circle-hover");
					}
				);
				$(".stage2-timeline-carousel .top-line-circle--1").mouseenter(
					function () {
						$(
							".stage2-timeline-carousel .top-line-circle--0"
						).addClass("timeline-circle-hover");
						$(
							".stage2-timeline-carousel .top-line-circle--1"
						).addClass("timeline-circle-hover");
						$(slickDates[0]).addClass("timeline-date-hover");
						$(slickDates[1]).addClass("timeline-date-hover");
						$(
							".stage2-timeline-carousel .carousel-top-line--one"
						).addClass("timeline-line-one-hover");
					}
				);
				$(".stage2-timeline-carousel .top-line-circle--1").mouseleave(
					function () {
						...
						$(
							".stage2-timeline-carousel .carousel-top-line--one"
						).removeClass("timeline-line-one-hover");
						$(
							".stage2-timeline-carousel .top-line-circle--1"
						).removeClass("timeline-circle-hover");
					}
				);
				$(".stage2-timeline-carousel .top-line-circle--2").mouseenter(
					function () {
						$(
							".stage2-timeline-carousel .top-line-circle--0"
						).addClass("timeline-circle-hover");
						...
						$(
							".stage2-timeline-carousel .top-line-circle--2"
						).addClass("timeline-circle-hover");
						$(slickDates[0]).addClass("timeline-date-hover");
						$(slickDates[1]).addClass("timeline-date-hover");
						...
						$(
							".stage2-timeline-carousel .carousel-top-line--two"
						).addClass("timeline-line-two-hover");
					}
				);
				$(".stage2-timeline-carousel .top-line-circle--2").mouseleave(
					function () {
						$(slickDates[0]).removeClass("timeline-date-hover");
						$(slickDates[1]).removeClass("timeline-date-hover");
						...
						$(
							".stage2-timeline-carousel .carousel-top-line--two"
						).removeClass("timeline-line-two-hover");
						$(
							".stage2-timeline-carousel .top-line-circle--2"
						).removeClass("timeline-circle-hover");
					}
			...
			$(slickDates[1]).mouseenter(function () {
				$(".stage2-timeline-carousel .top-line-circle--0").addClass(
					"timeline-circle-hover"
				);
				$(".stage2-timeline-carousel .top-line-circle--1").addClass(
					"timeline-circle-hover"
				);
				$(slickDates[0]).addClass("timeline-date-hover");
				$(slickDates[1]).addClass("timeline-date-hover");
				$(".stage2-timeline-carousel .carousel-top-line--one").addClass(
					"timeline-line-one-hover"
				);
			});
			$(slickDates[1]).mouseleave(function () {
				$(slickDates[1]).removeClass("timeline-date-hover");
				$(slickDates[0]).removeClass("timeline-date-hover");
				$(".stage2-timeline-carousel .top-line-circle--0").removeClass(
					"timeline-circle-hover"
				);
				$(
					".stage2-timeline-carousel .carousel-top-line--one"
				).removeClass("timeline-line-one-hover");
				$(".stage2-timeline-carousel .top-line-circle--1").removeClass(
					"timeline-circle-hover"
				);
			});
			$(slickDates[2]).mouseenter(function () {
				...
				$(".stage2-timeline-carousel .top-line-circle--2").addClass(
					"timeline-circle-hover"
				);
				$(slickDates[0]).addClass("timeline-date-hover");
				$(slickDates[1]).addClass("timeline-date-hover");
				...
				$(".stage2-timeline-carousel .carousel-top-line--two").addClass(
					"timeline-line-two-hover"
				);
			});
			$(slickDates[2]).mouseleave(function () {
				...
				$(".stage2-timeline-carousel .top-line-circle--2").removeClass(
					"timeline-circle-hover"
				);
			});
		}
...
function stage2HomeTimelineClickEffects() {
	...
	if ($(".stage2-timeline-carousel").length) {
		$(slickDates[0]).click(function () {
			...
			$(".stage2-timeline-carousel .carousel-top-line--one").removeClass(
				"timeline-line-one-active"
			);
			$(".stage2-timeline-carousel .top-line-circle--2").removeClass(
				"timeline-circle-active"
			);
			$(".stage2-timeline-carousel .carousel-top-line--two").removeClass(
				"timeline-line-two-active"
			);
		});
...
$(document).ready(function () {
	...
	function updateTabIndexForActiveSlide() {
		...
		$.js("timeline-carousel")
			.find(".carousel-text-container")
			.removeAttr("tabindex");
	...
	function setAriaCurrentForSlickActiveButton() {
		var allYearNavs = document.querySelectorAll(
			".stage2-timeline-carousel .slick-dots li button"
		);
		var activeSlide = $.js("timeline-carousel").find(".slick-active");
		...
		var activeSlideIndex = Array.from(
			activeSlide.parent().children()
		).indexOf(activeSlide[0]);
		var activeButton = allYearNavs[activeSlideIndex];
		var liveRegionText =
			"Slide " + (activeSlideIndex + 1) + " of " + allYearNavs.length;
		var liveRegion = document.getElementById("live-region");
		liveRegion.textContent = liveRegionText;
	}
	var timelineNavbuttons = document.querySelectorAll(
		".stage2-timeline-carousel .slick-dots li button"
	);
	...
	function carouselNewsPage() {
		...
		$(
			".station-navigation-list-buttons .navigation-buttons-section div"
		).unbind("keyup");
		$(
			".station-navigation-list-buttons .navigation-buttons-section div"
		).keyup(function (event) {
	...
	$(".photo-carousel-container-nav .navigation-buttons-slider.prev").click(
		function () {
			$(this)
				.closest(".container-photo-gallery-header")
				.find(".image-gallery-carousel")
				.slick("slickPrev");
		}
	);
	$(".photo-carousel-container-nav .navigation-buttons-slider.next").click(
		function () {
			$(this)
				.closest(".container-photo-gallery-header")
				.find(".image-gallery-carousel")
	...
	if ($(".en ul.stage2-breadcrumbs").length > 0) {
		...
		$("<p>></p>")
			.addClass("crumb-chevron")
			.appendTo(".en ul.stage2-breadcrumbs li");
	}
	if ($(".fr ul.stage2-breadcrumbs").length > 0) {
		$(".fr ul.stage2-breadcrumbs li:nth-child(2) a").text("Accueil");
		$("<p>></p>")
			.addClass("crumb-chevron")
			.appendTo(".fr ul.stage2-breadcrumbs li");
	}
